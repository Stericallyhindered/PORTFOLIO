...
export function DealerNavbar() {
  ...
  const handleSupportSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/dealers/support', {
        method: 'POST',
        headers: {
    ...
    <nav className="bg-black/70 border-b border-muted-foreground">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
                ...
                <DialogContent className="sm:max-w-[600px] fixed z-50">
                  <form onSubmit={handleSupportSubmit}>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Send a message to our dealer support team. We&apos;ll get back to you as
                        soon as possible.
                      </DialogDescription>
                    </DialogHeader>
          ...
          {/* Mobile menu button */}
          <h3 className="md:hidden flex text-white text-xl font-medium">Dealer Portal</h3>
