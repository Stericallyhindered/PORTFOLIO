...
struct HubConfig {
  ...
  // -- Tuya OpenAPI (optional cloud signing; LAN control still uses LocalTuya) --
  // Used by `TuyaClient` when calling Tuya cloud REST. Leave blank if unused.
  String tuyaClientId;
  String tuyaClientSecret;
  /// e.g. `https://openapi.tuyaus.com` (US) or `https://openapi.tuyaeu.com` (EU).
  String tuyaRegionHost;

  // -- Burst defaults (overridable via Supabase `setBurst` commands) ----------
  uint32_t burstOnSec  = 30;
  uint32_t burstOffSec = 150;

  bool hasTuyaCloud() const {
    return tuyaClientId.length() > 0 && tuyaClientSecret.length() > 0 &&
           tuyaRegionHost.length() > 0;
  }
