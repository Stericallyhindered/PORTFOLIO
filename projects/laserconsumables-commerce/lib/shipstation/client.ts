...
export class ShipStationClient {
  ...
  private async request(
    ...
    if (!response.ok) {
      ...
      let errorMessage = `ShipStation API error (${response.status}): ${errorText}`
      
      try {
        const errorJson = JSON.parse(errorText)
        if (errorJson.Message) {
          errorMessage = `ShipStation: ${errorJson.Message}`
...
export function getShipStationClient(): ShipStationClient {
  ...
  if (!apiKey || !apiSecret) {
    throw new Error('ShipStation API credentials not configured. Please add SHIPSTATION_API_KEY and SHIPSTATION_API_SECRET to environment variables.')
  }
