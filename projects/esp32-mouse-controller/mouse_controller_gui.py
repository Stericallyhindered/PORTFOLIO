...
class MouseControllerGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Control group
        control_group = QGroupBox("Recoil Control")
        control_layout = QVBoxLayout(control_group)
        
        # Auto mode checkbox
        self.auto_mode_cb = QCheckBox("Auto Mode - Monitor Left Mouse Button")
        self.auto_mode_cb.setStyleSheet("QCheckBox { color: #ffffff; }")
        self.auto_mode_cb.stateChanged.connect(self.toggle_auto_mode)
        control_layout.addWidget(self.auto_mode_cb)
        
        # Manual control buttons
        manual_layout = QHBoxLayout()
        self.start_btn = QPushButton("ENABLE")
        self.start_btn.setStyleSheet("QPushButton { background-color: #d13438; font-weight: bold; }")
        self.start_btn.clicked.connect(self.enable_recoil)
        self.start_btn.setEnabled(False)
        manual_layout.addWidget(self.start_btn)
        
        self.stop_btn = QPushButton("DISABLE")
        self.stop_btn.setStyleSheet("QPushButton { background-color: #107c10; font-weight: bold; }")
        self.stop_btn.clicked.connect(self.disable_recoil)
    ...
    def on_connection_status(self, connected):
        ...
        self.auto_mode_cb.setEnabled(connected)
    ...
    def enable_recoil(self):
        """Enable recoil compensation"""
        if self.is_connected:
            self.serial_worker.send_command("ENABLE")
            self.is_dragging = True
            self.start_btn.setEnabled(False)
            self.stop_btn.setEnabled(True)
            self.log_message("Recoil compensation enabled")
            
    def disable_recoil(self):
        """Disable recoil compensation"""
        if self.is_connected:
            self.serial_worker.send_command("DISABLE")
            self.is_dragging = False
            self.start_btn.setEnabled(True)
            self.stop_btn.setEnabled(False)
            self.log_message("Recoil compensation disabled")
            
    def toggle_auto_mode(self, state):
        """Toggle auto mode (monitor left mouse button)"""
        self.auto_mode = state == Qt.Checked
        if self.auto_mode:
            self.start_mouse_monitoring()
            self.log_message("Auto mode enabled - monitoring left mouse button")
        else:
    ...
    def start_mouse_monitoring(self):
        ...
        try:
            ...
            self.log_message(f"Error starting mouse monitoring: {e}")
    ...
    def on_mouse_click(self, x, y, button, pressed):
        ...
        if button == mouse.Button.left:
            if pressed and not self.is_dragging:
                # Left button pressed - enable recoil compensation
                self.enable_recoil()
            elif not pressed and self.is_dragging:
                # Left button released - disable recoil compensation
                self.disable_recoil()
